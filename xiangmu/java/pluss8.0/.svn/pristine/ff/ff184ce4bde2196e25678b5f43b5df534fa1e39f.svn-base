...
import cn.pluss.platform.model.annotation.Excel;
...
//            }
     ...
     * @param list      导出数据集合
     * @param sheetName 工作表的名称
     * @param uploadPath Excel存储的实际位置
     * @return 结果
     */
    public void exportExcel(List<?> list, String sheetName, String uploadPath) {
        HSSFWorkbook workbook = null;
...
//                    cellStyle.setWrapText(true);
                ...
                //一个sheet页最大的行数是65536，索引区间是[0, 65535]，{row number (65536) outside allowable range (0..65535)}
                //去除一行标题之后，第一个sheet页索引区间 [0,65534] 共 65535行， 第二个sheet页最大索引为[65535, 131069] ，故左区间 startNo = index * (sheetSize - 1)
                int startNo = index * (sheetSize - 1);
                int endNo = Math.min(startNo + sheetSize - 1, list.size());
                // 写入各条记录,每条记录对应excel表中的一行
...
//            workbook.write(response.getOutputStream());
            ...
            out = new FileOutputStream(uploadPath + "/" + sheetName);
            workbook.write(out);
        } catch (Exception e) {
            log.error("导出Excel异常{}", e.getMessage());
            throw new RuntimeException("导出Excel失败，请联系网站管理员！");
    ...
    /**
     * 对list数据源将其里面的数据导入到excel表单，导出XLSX后缀的Excel文件
     *
     * @param list      导出数据集合
     * @param sheetName 工作表的名称
     * @param uploadPath Excel存储的实际位置
     * @return 结果
     */
    public void exportExcelForXlsx(List<?> list, String sheetName, String uploadPath) {
...
//                    cellStyle.setWrapText(true);
     ...
     * @return 设置好的sheet.
     */
    private static XSSFSheet setXSSFPrompt(XSSFSheet sheet, String promptTitle, String promptContent, int firstRow,
                                           int endRow, int firstCol, int endCol) {
     ...
     * @return 设置好的sheet.
     */
    private static XSSFSheet setXSSFValidation(XSSFSheet sheet, String[] textlist, int firstRow, int endRow,
                                               int firstCol, int endCol) {
