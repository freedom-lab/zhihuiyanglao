					...
					<view class="merchant-item" @click="toAudit(item)">
						...
						<view class="merchant-info">
							...
							<view class="info-mid">
								<view class="mid-left">
									<!-- 地址 -->
									<text class="u-line-2 merchant-address">{{ item.merchantShortAddress }}</text>
...
<script>
	...
	export default {
		...
		onShow() {
			const justapproved = uni.getStorageSync('justapproved')
			if (justapproved) {
				uni.setStorageSync("justapproved", false)
				this.init()
			}
		},
		onPullDownRefresh() {
		...
		methods: {
			...
			async getAuditList() {
				...
				const res = await this.$request("/merchantApproval/list", params)
			...
			toAudit(e) {
				console.log('审核页：', e)
				uni.navigateTo({
					url: `/subPages/merchantaudit/merchantaudit?merchantCode=${e.merchantCode}`
				})
			},
		}
	}
