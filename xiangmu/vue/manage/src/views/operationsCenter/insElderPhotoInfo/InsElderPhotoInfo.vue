  ...
  <div style="padding:16px">
    ...
    <div ref="search_d">
      <el-row type="flex" class="row-bg" justify="space-around">
        <el-col :span="15">
          <span style="font-size:18px;font-weight:bold">老人生活相册</span>
        </el-col>
        <el-col :span="10">
    ...
    <div class="table-box">
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%"
        ...
        <!-- 相册业务信息 -->
        <el-table-column label="相册主题" align="center" min-width="140" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.titleName }}</template>
        </el-table-column>
        <el-table-column label="相册日期" align="center" width="110" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.titleDate ? scope.row.titleDate.split(' ')[0] : '' }}</template>
        </el-table-column>
        <el-table-column label="相册文件路径" align="center" min-width="220" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.photoPath }}</template>
        </el-table-column>
      ...
      <el-container class="dialogBox">
        ...
        <el-main class="dialogMain">
          <el-form ref="form" :model="form" :rules="rules" label-width="120px">
            ...
            <!-- 相册核心业务 -->
            <el-col :span="12">
              <el-form-item label="相册主题" prop="titleName">
                <el-input v-model="form.titleName" placeholder="填写相册主题名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="相册日期" prop="titleDate">
                <el-date-picker v-model="form.titleDate" type="date" value-format="yyyy-MM-dd" placeholder="选择相册拍摄日期"
                  style="width:100%" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="相册文件路径" prop="photoPath">
                <el-input v-model="form.photoPath" placeholder="多个文件路径使用英文逗号分隔" />
              </el-form-item>
            </el-col>
...
export default {
  ...
  methods: {
    ...
    getList() {
      var that = this;
      that.loading = true
      that.$http.post("/insElderPhotoInfo/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      var that = this;
      that.$http.post("/insElderPhotoInfo/save", that.form).then(function (response) {
        if (response.data.code == 200) {
          that.$notify.success({
            title: "提示",
            message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/insElderPhotoInfo/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
