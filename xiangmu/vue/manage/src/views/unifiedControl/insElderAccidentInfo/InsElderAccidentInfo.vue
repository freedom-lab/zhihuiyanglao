    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="事故类型" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.accidentType }}</template>
          </el-table-column>
          <el-table-column label="事故原因" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.accidentReason }}</template>
          </el-table-column>
          <el-table-column label="责任人id" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.nursingStaffId }}</template>
          </el-table-column>
          <el-table-column label="责任人姓名" align="center" show-overflow-tooltip>
          ...
          <el-table-column label="处理结果" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.handlingResult }}</template>
          </el-table-column>
        ...
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="发生时间：" prop="siteName">
                <el-input v-model="form.occurTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="事故类型：" prop="siteName">
                <el-input v-model="form.accidentType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="事故原因：" prop="siteName">
                <el-input v-model="form.accidentReason" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            
            <el-col :span="12">
              <el-form-item label="责任人姓名：" prop="siteName">
                <el-input v-model="form.nursingStaffName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="处理结果：" prop="siteName">
                <el-input v-model="form.handlingResult" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insElderAccidentInfo/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insElderAccidentInfo/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insElderAccidentInfo/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
