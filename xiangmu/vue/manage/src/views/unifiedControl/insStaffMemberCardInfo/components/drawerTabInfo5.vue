    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="状态" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.evaluationState | evaluationStateFormat }}</template>
        </el-table-column>
        <el-table-column label="考评类型" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.evaluationType | evaluationTypeFormat }}</template>
        </el-table-column>
  ...
  methods: {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insStaffEvaluationRecord/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
