    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="投诉类型" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.complaintType }}</template>
          </el-table-column>
          <el-table-column label="投诉人" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.complaintName }}</template>
          </el-table-column>
          ...
          <el-table-column label="投诉时间" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.complaintTime }}</template>
          </el-table-column>
          <el-table-column label="投诉内容" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.complaintContent }}</template>
          </el-table-column>
          ...
          <el-table-column label="处理人" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.handleName }}</template>
          </el-table-column>
          <el-table-column label="处理结果" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.handleResults }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="投诉类型：" prop="siteName">
                <el-input v-model="form.complaintType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="投诉人：" prop="siteName">
                <el-input v-model="form.complaintName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="投诉时间：" prop="siteName">
                <el-input v-model="form.complaintTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="投诉内容：" prop="siteName">
                <el-input v-model="form.complaintContent" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="处理人：" prop="siteName">
                <el-input v-model="form.handleName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insSuddenComplaintRecord/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insSuddenComplaintRecord/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insSuddenComplaintRecord/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
