  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="费用名称" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.itemName }}</template>
        </el-table-column>
        <el-table-column label="费用类型" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.costType | costTypeFormat }}</template>
        </el-table-column>
        <el-table-column label="应收金额（元）" align="center" min-width="130">
          <template slot-scope="scope">{{ scope.row.itemPrice }}</template>
        </el-table-column>
        <el-table-column label="实收金额（元）" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.discountedPrice }}</template>
        </el-table-column>
        <el-table-column label="费用状态" align="center" min-width="80">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.post("/insExpensesElderCostStandard/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insExpensesElderCostStandard/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/insExpensesElderCostStandard/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
