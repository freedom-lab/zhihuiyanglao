  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="体检日期" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.reportTime }}</template>
        </el-table-column>
        <el-table-column label="体检医院名称" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.hospitalName }}</template>
        </el-table-column>
        <el-table-column label="体检报告类型" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.reportType | reportTypeFormat }}</template>
        </el-table-column>
        <el-table-column label="附件照片" align="center" min-width="150" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-image v-for="item,index in scope.row.reportPathList" :key="index"
              :src="item" style="width: 60px;height: 60px;" :preview-src-list="[scope.row.reportPathList]"
            fit="contain">
...
export default {
  ...
  data() {
    return {
      ...
      showDialog: false,
      tableData: [],
      height: 320,//window.innerHeight - 556, //表格高度
      headerHeight: 0,
      loading: false,
    };
  },
  ...
  methods: {
      ...
      that.$http.post("/insHealthPhysicalReport/list", that.queryInfo).then(function (response) {
        ...
        if (response.data.code == 200) {
          ...
          that.tableData = response.data.data.data.map(item=>{
            item.reportPathList = item.reportPath.split(',')
            return item
          });
        }
      });
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insHealthPhysicalReport/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
