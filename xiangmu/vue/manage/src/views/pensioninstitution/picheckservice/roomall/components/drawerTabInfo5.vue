  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="合同名称" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.contractName }}</template>
        </el-table-column>
        <el-table-column label="签订时间" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.signTime }}</template>
        </el-table-column>
        <el-table-column label="合同状态" align="center" min-width="80">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.post("/insElderContractInfo/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insElderContractInfo/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/insElderContractInfo/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
