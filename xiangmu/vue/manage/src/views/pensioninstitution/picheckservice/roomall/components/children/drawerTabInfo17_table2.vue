  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="购买时间" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.orderTime }}</template>
        </el-table-column>
        <el-table-column label="订单状态" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.orderState | orderStateFormat }}</template>
        </el-table-column>
        <el-table-column label="订单总金额（元）" align="center" min-width="120">
        ...
        <el-table-column label="付款状态" align="center" min-width="80">
  ...
  methods: {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insLogisticsOrderInfo/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
