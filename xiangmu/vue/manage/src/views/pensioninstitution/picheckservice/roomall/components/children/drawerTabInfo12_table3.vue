  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="原餐费名称" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.oldFoodName }}</template>
        </el-table-column>
        <el-table-column label="原餐费金额（元）" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.oldFoodPrice }}</template>
        </el-table-column>
        <el-table-column label="新餐费名称" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.newFoodName }}</template>
        </el-table-column>
        <el-table-column label="新餐费金额（元）" align="center" min-width="120">
  ...
  methods: {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insExpensesChangeFoodRecord/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
