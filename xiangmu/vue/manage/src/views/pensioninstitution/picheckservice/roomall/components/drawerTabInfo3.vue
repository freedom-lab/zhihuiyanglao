  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="亲属姓名" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.relativesName }}</template>
        </el-table-column>
        <el-table-column label="性别" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.sex == 1 ? '男' : '女' }}</template>
        </el-table-column>
        <el-table-column label="亲属电话" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.linkTel }}</template>
        </el-table-column>
        <el-table-column label="与老人关系" align="center" min-width="130">
          <template slot-scope="scope">{{ scope.row.relationshipType }}</template>
        </el-table-column>
    ...
    <el-dialog :title="labelType=='add' ? '新增' : '编辑'" class="reservation-dialog" append-to-body :visible.sync="showDialog" width="75%" top="15vh">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="亲属姓名" prop="relativesName">
              <el-input v-model="form.relativesName" placeholder="请输入" clearable></el-input>
            </el-form-item>
          </el-col>
          <!-- 第一行 右 -->
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <!-- 第二行 -->
          <el-col :span="12">
            <el-form-item label="亲属电话" prop="linkTel">
              <el-input v-model="form.linkTel" placeholder="请输入" clearable></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="与老人关系" prop="relationshipType">
              <el-input v-model="form.relationshipType" placeholder="请输入" clearable></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.post("/insElderRelativesBinding/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insElderRelativesBinding/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/insElderRelativesBinding/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
