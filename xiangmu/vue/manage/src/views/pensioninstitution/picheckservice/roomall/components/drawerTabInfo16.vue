  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="请假状态" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.status | statusFormat }}</template>
        </el-table-column>
        <el-table-column label="请假日期" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.leaveDate }}</template>
        </el-table-column>
        <el-table-column label="销假日期" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.backDate }}</template>
        </el-table-column>
        <el-table-column label="请假天数" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.leaveDay }}</template>
        </el-table-column>
        <el-table-column label="陪同人" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.withName }}</template>
        </el-table-column>
        
        <el-table-column label="陪同人联系电话" align="center" min-width="130">
          <template slot-scope="scope">{{ scope.row.withLinkTel }}</template>
        </el-table-column>
        <!-- <el-table-column label="与老人关系" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.intubationType }}</template>
        </el-table-column> -->
        <el-table-column label="请假原因" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.leaveReason }}</template>
        </el-table-column>
        <el-table-column label="是否退床位费" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.isRefund | yonFormat }}</template>
        </el-table-column>
        <el-table-column label="办理人" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.createName }}</template>
        </el-table-column>
        <el-table-column label="办理日期" align="center" min-width="130">
          <template slot-scope="scope">{{ scope.row.createTime }}</template>
        </el-table-column>
        <el-table-column label="审核状态" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.calculateState | calculateStateFormat }}</template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.post("/careLeaveRecord/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/careLeaveRecord/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/careLeaveRecord/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
