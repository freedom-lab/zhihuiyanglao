    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        ...
        <el-table-column label="老人姓名" align="center" show-overflow-tooltip>
          ...
          <el-table-column label="计划翻身时间(hh:mm)" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.planTime }}</template>
          </el-table-column>
          <el-table-column label="实际翻身时间(hh:mm)" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.realityTime }}</template>
          </el-table-column>
          <el-table-column label="卧位(左卧/右卧/平卧/坐位)" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.decubitus }}</template>
          </el-table-column>
          <el-table-column label="皮肤情况" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.skinCondition }}</template>
          </el-table-column>
          <el-table-column label="防护用具" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.protectiveEquipment }}</template>
          </el-table-column>
          <el-table-column label="压疮处理" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.pressureUlcer }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="计划翻身时间(hh:mm)：" prop="siteName">
                <el-input v-model="form.planTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="实际翻身时间(hh:mm)：" prop="siteName">
                <el-input v-model="form.realityTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="卧位(左卧/右卧/平卧/坐位)：" prop="siteName">
                <el-input v-model="form.decubitus" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="皮肤情况：" prop="siteName">
                <el-input v-model="form.skinCondition" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="防护用具：" prop="siteName">
                <el-input v-model="form.protectiveEquipment" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="压疮处理：" prop="siteName">
                <el-input v-model="form.pressureUlcer" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insNursingTurnoverRecord/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insNursingTurnoverRecord/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insNursingTurnoverRecord/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
