    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="问诊时间" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.consultationTime }}</template>
          </el-table-column>
          <el-table-column label="现病史" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.medicalHistory }}</template>
          </el-table-column>
          <el-table-column label="主诉内容" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.mainContent }}</template>
          </el-table-column>
          <el-table-column label="问诊结果" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.checkResult }}</template>
          </el-table-column>
         
          <el-table-column label="医生姓名" align="center" show-overflow-tooltip>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="问诊时间：" prop="siteName">
                <el-input v-model="form.consultationTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="现病史：" prop="siteName">
                <el-input v-model="form.medicalHistory" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="主诉内容：" prop="siteName">
                <el-input v-model="form.mainContent" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="问诊结果：" prop="siteName">
                <el-input v-model="form.checkResult" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="医生id：" prop="siteName">
                <el-input v-model="form.doctorId" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="医生姓名：" prop="siteName">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insMedicalConsultationRecord/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insMedicalConsultationRecord/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insMedicalConsultationRecord/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
