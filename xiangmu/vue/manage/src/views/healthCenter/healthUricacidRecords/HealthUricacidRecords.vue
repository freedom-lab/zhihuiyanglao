    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="尿酸值" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.uricAcid }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="性别(1：男，2：女)：" prop="siteName">
            ...
            <el-col :span="12">
              <el-form-item label="测量条件(0餐后测量1餐前测量2随机测量)：" prop="siteName">
                <el-input v-model="form.conditions" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="尿酸值：" prop="siteName">
                <el-input v-model="form.uricAcid" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="分析结果(0正常，1偏低，2偏高)：" prop="siteName">
            ...
            <el-col :span="12">
              <el-form-item label="预警标记(0未扫描1已扫描(默认0))：" prop="siteName">
            ...
            <el-col :span="12">
              <el-form-item label="业务字段：" prop="siteName">
                <el-input v-model="form.status" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="数据来源(字典值)：" prop="siteName">
                <el-input v-model="form.dataSource" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/healthUricacidRecords/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/healthUricacidRecords/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/healthUricacidRecords/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
