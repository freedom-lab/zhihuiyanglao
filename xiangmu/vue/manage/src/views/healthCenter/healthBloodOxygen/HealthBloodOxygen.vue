    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="血氧饱和度" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.oxygenSaturation }}</template>
          </el-table-column>
          ...
          <el-table-column label="体温" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.bodyTemperature }}</template>
          </el-table-column>
...
<!-- 
            ...
            <el-col :span="12">
              <el-form-item label="血氧饱和度：" prop="siteName">
                <el-input v-model="form.oxygenSaturation" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="体温：" prop="siteName">
                <el-input v-model="form.bodyTemperature" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/healthBloodOxygen/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/healthBloodOxygen/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/healthBloodOxygen/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
