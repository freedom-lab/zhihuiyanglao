      ...
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="健康类型" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.healthyType }}</template>
          </el-table-column>
          <el-table-column label="预警内容" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.warningText }}</template>
          </el-table-column>
          <el-table-column label="预警时间" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.warningTime }}</template>
          </el-table-column>
        ...
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="健康类型：" prop="siteName">
                <el-input v-model="form.healthyType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="预警内容：" prop="siteName">
                <el-input v-model="form.warningText" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="预警时间：" prop="siteName">
                <el-input v-model="form.warningTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/healthWarningLog/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/healthWarningLog/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/healthWarningLog/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
