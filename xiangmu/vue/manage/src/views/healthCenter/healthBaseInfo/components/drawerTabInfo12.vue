  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="评估类型" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.assessMemo | assessMemoFormat }}</template>
        </el-table-column>
        <el-table-column label="评估结果" align="center" min-width="100">
          <el-table-column label="初步结果" align="center" min-width="100">
            <template slot-scope="scope">{{ scope.row.initialLevel | assessLevelFormat }}</template>
          </el-table-column>
          <el-table-column label="最终结果" align="center" min-width="100">
            <template slot-scope="scope">{{ scope.row.assessResult | assessLevelFormat }}</template>
          </el-table-column>
        </el-table-column>
        <el-table-column label="评估人员" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.assignStaffName }}</template>
        </el-table-column>
        <el-table-column label="评估时间" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.assessTime }}</template>
        </el-table-column>
...
export default {
  ...
  filters: {
    // 评估类型 评估原因(1 首次评估 2 常规评估 3 因评估结果有疑问进行的复评 4其他)
    assessMemoFormat(val){
      switch (val) {
        case 1:
        case '1':
          return '首次评估';
    ...
    // 评估结果 0能力完好1轻度失能2中度失能3重度失能4完全失能
    assessLevelFormat(val){
      switch (val) {
        case 0:
        case '0':
          return '能力完好';
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.post("/assessmentAppoint/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/assessmentAppoint/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
