  ...
  <div style="padding:16px">
    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="资源编号" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.smsCode }}</template>
          </el-table-column>
          <el-table-column label="accessKey" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.accessKey }}</template>
          </el-table-column>
          <el-table-column label="secretKey" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.secretKey }}</template>
          </el-table-column>
          <el-table-column label="regionId" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.regionId }}</template>
          </el-table-column>
          <el-table-column label="短信签名" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.signName }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            <el-col :span="12">
              <el-form-item label="分类：" prop="siteName">
                <el-input v-model="form.category" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="资源编号：" prop="siteName">
                <el-input v-model="form.smsCode" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="accessKey：" prop="siteName">
                <el-input v-model="form.accessKey" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="secretKey：" prop="siteName">
                <el-input v-model="form.secretKey" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="regionId：" prop="siteName">
                <el-input v-model="form.regionId" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="短信签名：" prop="siteName">
                <el-input v-model="form.signName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/bladeSms/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/bladeSms/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/bladeSms/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
