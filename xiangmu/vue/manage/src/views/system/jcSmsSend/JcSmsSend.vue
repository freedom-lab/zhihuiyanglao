    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="接收方手机号码" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.telNum }}</template>
          </el-table-column>
          <el-table-column label="短信模板ID" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.templetId }}</template>
          </el-table-column>
          <el-table-column label="短信内容" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.smsContext }}</template>
          </el-table-column>
          <el-table-column label="发送状态" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.sendState }}</template>
          </el-table-column>
          <el-table-column label="发送结果(从短信服务器返回)" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.sendResult }}</template>
          </el-table-column>
          <el-table-column label="短信发送时间" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.sendTime }}</template>
          </el-table-column>
          <el-table-column label="发送类型(1主动关怀" align="center" show-overflow-tooltip>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="接收方手机号码：" prop="siteName">
                <el-input v-model="form.telNum" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="短信内容：" prop="siteName">
                <el-input v-model="form.smsContext" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="发送状态：" prop="siteName">
                <el-input v-model="form.sendState" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="发送结果(从短信服务器返回)：" prop="siteName">
                <el-input v-model="form.sendResult" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="短信发送时间：" prop="siteName">
                <el-input v-model="form.sendTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="发送类型(1主动关怀：" prop="siteName">
                <el-input v-model="form.sendType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="IP地址：" prop="siteName">
                <el-input v-model="form.ip" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/jcSmsSend/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/jcSmsSend/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/jcSmsSend/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
