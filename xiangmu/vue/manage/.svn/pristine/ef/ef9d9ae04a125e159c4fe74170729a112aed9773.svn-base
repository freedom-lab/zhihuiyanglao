    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="药品包装量" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.drugPackageNum }}</template>
          </el-table-column>
          <el-table-column label="药品总量" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.drugNum }}</template>
          </el-table-column>
          <el-table-column label="药品最小服用单位" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.drugUnit }}</template>
          </el-table-column>
          <el-table-column label="服用方法" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.drugUsage }}</template>
          </el-table-column>
          <el-table-column label="有效期" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.effectiveDate }}</template>
          </el-table-column>
          <el-table-column label="送药人" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.deliveryPerson }}</template>
          </el-table-column>

          <el-table-column label="接收人姓名" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.staffName }}</template>
          </el-table-column>
          <el-table-column label="包装是否完好" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.isIntact }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="药品包装量：" prop="siteName">
                <el-input v-model="form.drugPackageNum" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="药品总量：" prop="siteName">
                <el-input v-model="form.drugNum" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="药品最小服用单位：" prop="siteName">
                <el-input v-model="form.drugUnit" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="服用方法：" prop="siteName">
                <el-input v-model="form.drugUsage" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="送药人：" prop="siteName">
                <el-input v-model="form.deliveryPerson" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="接收人id：" prop="siteName">
                <el-input v-model="form.staffId" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="接收人姓名：" prop="siteName">
                <el-input v-model="form.staffName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="包装是否完好：" prop="siteName">
                <el-input v-model="form.isIntact" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insMedicalSelfDrug/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insMedicalSelfDrug/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insMedicalSelfDrug/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
