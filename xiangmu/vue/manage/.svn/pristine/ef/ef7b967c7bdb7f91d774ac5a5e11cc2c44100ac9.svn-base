    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="不合规类别" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.errorType }}</template>
          </el-table-column>
          <el-table-column label="发生时间" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.errorDate }}</template>
          </el-table-column>
          <el-table-column label="不合规位置" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.errorLocation }}</template>
          </el-table-column>
          <el-table-column label="不合规说明" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.errorMemo }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="不合规类别：" prop="siteName">
                <el-input v-model="form.errorType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="发生时间：" prop="siteName">
                <el-input v-model="form.errorDate" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="不合规位置：" prop="siteName">
                <el-input v-model="form.errorLocation" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="不合规说明：" prop="siteName">
                <el-input v-model="form.errorMemo" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="不合规截图：" prop="siteName">
                <el-input v-model="form.errorImagePath" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="后续追踪情况：" prop="siteName">
            ...
            <!-- <el-col :span="12">
              <el-form-item label="上报人id：" prop="siteName">
                <el-input v-model="form.createUser" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col> -->
            <el-col :span="12">
              <el-form-item label="上报人名称：" prop="siteName">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insSuddenErrorInfo/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insSuddenErrorInfo/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insSuddenErrorInfo/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
