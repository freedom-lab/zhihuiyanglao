    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        ...
        <el-table-column label="入住日期" align="center" show-overflow-tooltip>
          ...
          <el-table-column label="健康类型" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.healthType }}</template>
          </el-table-column>
          <el-table-column label="最高值" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.maxValue }}</template>
          </el-table-column>
          <el-table-column label="最低值" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.minValue }}</template>
          </el-table-column>
          <el-table-column label="最高值" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.maxTwoValue }}</template>
          </el-table-column>
          <el-table-column label="最低值" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.minTwoValue }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="健康类型：" prop="siteName">
                <el-input v-model="form.healthType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="最高值：" prop="siteName">
                <el-input v-model="form.maxValue" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="最低值：" prop="siteName">
                <el-input v-model="form.minValue" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="最高值(血压低压/血糖餐后)：" prop="siteName">
                <el-input v-model="form.maxTwoValue" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="最低值(血压低压/血糖餐后)：" prop="siteName">
                <el-input v-model="form.minTwoValue" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insElderHealthParam/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insElderHealthParam/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insElderHealthParam/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
