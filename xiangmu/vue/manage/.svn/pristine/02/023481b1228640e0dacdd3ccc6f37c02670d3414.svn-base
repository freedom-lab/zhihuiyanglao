  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="胆固醇含量" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.chol }}</template>
        </el-table-column>
        <el-table-column label="高密度脂蛋白胆固醇含量" align="center" width="120" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.hdlchol }}</template>
        </el-table-column>
        <el-table-column label="甘油三酯含量" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.trig }}</template>
        </el-table-column>
        <el-table-column label="低密度脂蛋白胆固醇含量" align="center" width="120" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.calcldl }}</template>
        </el-table-column>
        <el-table-column label="总胆固醇和高密度胆固醇的比值" align="center" width="120" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.tcHdl }}</template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.post("/healthBloodFat/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/healthBloodFat/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/healthBloodFat/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
