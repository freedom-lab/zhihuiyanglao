    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="请假状态" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.leaveState }}</template>
          </el-table-column>
          <el-table-column label="请假时间" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.startTime }}</template>
          </el-table-column>
          <el-table-column label="销假时间" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.realityTime }}</template>
          </el-table-column>
          <el-table-column label="请假天数" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.leaveDay }}</template>
          </el-table-column>
          <el-table-column label="请假事由" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.leaveMemo }}</template>
          </el-table-column>
...
<!-- 
            ...
            <el-col :span="12">
              <el-form-item label="请假时间：" prop="siteName">
                <el-input v-model="form.startTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="销假时间：" prop="siteName">
                <el-input v-model="form.realityTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="请假天数：" prop="siteName">
                <el-input v-model="form.leaveDay" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="请假事由：" prop="siteName">
                <el-input v-model="form.leaveMemo" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insStaffLeaveRecord/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insStaffLeaveRecord/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insStaffLeaveRecord/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
