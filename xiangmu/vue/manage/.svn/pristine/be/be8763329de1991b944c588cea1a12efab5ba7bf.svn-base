  ...
  <div class="managingPeople hp100 bbox " v-loading="loading">
    ...
    <el-card>
      ...
      >
        ...
        <el-table-column label="key值" align="center" prop="key值">
          <template slot-scope="scope">
            <span>{{ scope.row.propertyKey }}</span>
          </template>
        </el-table-column>
        <el-table-column label="value值" align="center" prop="value值">
          <template slot-scope="scope">
            <span>{{ scope.row.propertyValue }}</span>
          </template>
        </el-table-column>
        <el-table-column label="属性描述" align="center" prop="属性描述">
          <template slot-scope="scope">
            <span>{{ scope.row.propertyDesc }}</span>
          </template>
        </el-table-column>
        <el-table-column label="排序" align="center" prop="排序">
          <template slot-scope="scope">
            <span>{{ scope.row.propertyIndex }}</span>
          </template>
        </el-table-column>
        <el-table-column label="归属系统" align="center" prop="归属系统">
          <template slot-scope="scope">
            <span>{{ scope.row.systemId }}</span>
          </template>
        </el-table-column>

        <el-table-column label="创建日期" align="center" prop="创建日期">
          <template slot-scope="scope">
  ...
  methods: {
    ...
    async del(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http
          .post("/systemConfig/delete", { id: id })
    ...
    getListData() {
      ...
      var that = this;
      that.$http
        .post("/systemConfig/list", that.queryInfo)
        .then(function(response) {
