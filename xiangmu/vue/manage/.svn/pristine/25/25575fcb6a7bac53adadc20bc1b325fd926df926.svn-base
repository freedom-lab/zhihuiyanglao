    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="采购日期" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.procureDate }}</template>
          </el-table-column>
          <el-table-column label="采购编号" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.procureCode }}</template>
          </el-table-column>
          <el-table-column label="入库类型" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.rkType }}</template>
          </el-table-column>
          ...
          <el-table-column label="采购人" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.procureUser }}</template>
          </el-table-column>
          <el-table-column label="采购金额(元)" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.procureAmount }}</template>
          </el-table-column>
          <el-table-column label="入库状态" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.rkState }}</template>
          </el-table-column>
...
<!-- 
            ...
            <el-col :span="12">
              <el-form-item label="采购日期：" prop="siteName">
                <el-input v-model="form.procureDate" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="采购编号：" prop="siteName">
                <el-input v-model="form.procureCode" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="入库类型：" prop="siteName">
                <el-input v-model="form.rkType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="12">
              <el-form-item label="采购人：" prop="siteName">
                <el-input v-model="form.procureUser" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="采购金额(元)：" prop="siteName">
                <el-input v-model="form.procureAmount" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="入库状态：" prop="siteName">
                <el-input v-model="form.rkState" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insLogisticsProcureInfo/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insLogisticsProcureInfo/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insLogisticsProcureInfo/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
