  ...
  <div style="padding:16px">
    ...
    <div ref="search_d">
      <el-row type="flex" class="row-bg" justify="space-around">
        <el-col :span="15">
          <span style="font-size:18px;font-weight:bold">床位变更申请</span>
        </el-col>
        <el-col :span="10">
    ...
    <div class="table-box">
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%"
        ...
        <el-table-column label="是否包房" align="center" width="80" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.isRoom === 1 ? '否' : '是' }}</template>
        </el-table-column>

        <!-- 3. 原床位信息 -->
        <el-table-column label="原楼栋" align="center" min-width="100" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.oldBuildingName }}</template>
        </el-table-column>
        <el-table-column label="原楼层" align="center" width="80" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.oldFloorName }}</template>
        </el-table-column>
        <el-table-column label="原房间" align="center" width="80" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.oldRoomName }}</template>
        </el-table-column>
        <el-table-column label="原床位" align="center" width="80" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.oldBedName }}</template>
        </el-table-column>
        <el-table-column label="原床位价格" align="center" width="100" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.oldBedPrice }}</template>
        </el-table-column>
        <el-table-column label="原床位信息" align="center" min-width="150" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.oldBedInfo }}</template>
        </el-table-column>

        <!-- 4. 新床位信息 -->
        <el-table-column label="新楼栋" align="center" min-width="100" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.newBuildingName }}</template>
        </el-table-column>
        <el-table-column label="新楼层" align="center" width="80" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.newFloorName }}</template>
        </el-table-column>
        <el-table-column label="新房间" align="center" width="80" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.newRoomName }}</template>
        </el-table-column>
        <el-table-column label="新床位" align="center" width="80" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.newBedName }}</template>
        </el-table-column>
        <el-table-column label="新床位价格" align="center" width="100" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.newBedPrice }}</template>
        </el-table-column>
        <el-table-column label="新床位信息" align="center" min-width="150" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.newBedInfo }}</template>
        </el-table-column>
          ...
          <el-form ref="form" :model="form" :rules="rules" label-width="120px">
            ...
            <el-col :span="12">
              <el-form-item label="是否包房" prop="isRoom">
                <el-input v-model="form.isRoom" placeholder="1=否，2=是" />
              </el-form-item>
            </el-col>

            <!-- 3. 原床位信息 -->
            <el-col :span="12">
              <el-form-item label="原楼栋" prop="oldBuildingName">
                <el-input v-model="form.oldBuildingName" placeholder="原楼栋名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="原楼层" prop="oldFloorName">
                <el-input v-model="form.oldFloorName" placeholder="原楼层名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="原房间" prop="oldRoomName">
                <el-input v-model="form.oldRoomName" placeholder="原房间名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="原床位" prop="oldBedName">
                <el-input v-model="form.oldBedName" placeholder="原床位名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="原床位价格" prop="oldBedPrice">
                <el-input v-model="form.oldBedPrice" placeholder="原床位价格（元）" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="原床位信息" prop="oldBedInfo">
                <el-input v-model="form.oldBedInfo" placeholder="原床位补充信息" />
              </el-form-item>
            </el-col>

            <!-- 4. 新床位信息 -->
            <el-col :span="12">
              <el-form-item label="新楼栋" prop="newBuildingName">
                <el-input v-model="form.newBuildingName" placeholder="新楼栋名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="新楼层" prop="newFloorName">
                <el-input v-model="form.newFloorName" placeholder="新楼层名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="新房间" prop="newRoomName">
                <el-input v-model="form.newRoomName" placeholder="新房间名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="新床位" prop="newBedName">
                <el-input v-model="form.newBedName" placeholder="新床位名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="新床位价格" prop="newBedPrice">
                <el-input v-model="form.newBedPrice" placeholder="新床位价格（元）" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="新床位信息" prop="newBedInfo">
                <el-input v-model="form.newBedInfo" placeholder="新床位补充信息" />
              </el-form-item>
            </el-col>
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      var that = this;
      that.$http.post("/insElderChangeBedRecord/save", that.form).then(function (response) {
        if (response.data.code == 200) {
          that.$notify.success({
            title: "提示",
            message: "保存成功",
