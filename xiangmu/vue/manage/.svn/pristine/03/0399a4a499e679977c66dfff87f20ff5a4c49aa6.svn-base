    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="心脏功能" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.veinOne }}</template>
          </el-table-column>
          <el-table-column label="血管状态" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.veinTwo }}</template>
          </el-table-column>
          <el-table-column label="血液状态" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.veinThere }}</template>
          </el-table-column>
          <el-table-column label="微循环功能" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.veinFour }}</template>
          </el-table-column>
          <el-table-column label="人体运动参数" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.veinFive }}</template>
          </el-table-column>
          <el-table-column label="测量结果" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.veinResult }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="心脏功能：" prop="siteName">
                <el-input v-model="form.veinOne" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="血管状态：" prop="siteName">
                <el-input v-model="form.veinTwo" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="血液状态：" prop="siteName">
                <el-input v-model="form.veinThere" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="微循环功能：" prop="siteName">
                <el-input v-model="form.veinFour" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="人体运动参数：" prop="siteName">
                <el-input v-model="form.veinFive" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="测量结果：" prop="siteName">
                <el-input v-model="form.veinResult" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/healthCardiovascular/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/healthCardiovascular/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/healthCardiovascular/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
