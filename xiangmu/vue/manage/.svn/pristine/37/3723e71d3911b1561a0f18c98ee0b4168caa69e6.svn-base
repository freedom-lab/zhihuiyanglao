  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="护理项目" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.itemName }}</template>
        </el-table-column>
        <el-table-column label="状态" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.sex | sexFormat }}</template>
        </el-table-column>
        <el-table-column label="操作类型" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.operationType }}</template>
        </el-table-column>
        <el-table-column label="项目类型" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.itemCategory | itemCategoryFormat }}</template>
        </el-table-column>
        <el-table-column label="项目频率" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.itemFrequency | itemFrequencyFormat }}</template>
        </el-table-column>
        <el-table-column label="服务时间" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.itemFrequency ==1? `${scope.row.startTime}-${scope.row.endTime}` :scope.row.itemFrequency ==2? scope.row.weekNum :scope.row.dayNum}}</template>
        </el-table-column>
        <el-table-column label="服务说明" align="center" min-width="120">
  ...
  methods: {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/careNursingElderStandard/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
