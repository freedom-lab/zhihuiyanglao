    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="离职类型" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.dimissionType | dimissionTypeFormat }}</template>
        </el-table-column>
  ...
  methods: {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insStaffDimissionRecord/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
