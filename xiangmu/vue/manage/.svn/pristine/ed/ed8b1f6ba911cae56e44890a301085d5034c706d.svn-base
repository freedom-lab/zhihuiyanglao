  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="老人姓名" align="center" min-width="120" fixed="left">
          <template slot-scope="scope">{{ scope.row.elderName }}</template>
        </el-table-column>
        <el-table-column label="性别" align="center" min-width="80">
          <template slot-scope="scope">{{ elderForm.sex | sexFormat }}</template>
        </el-table-column>
        <el-table-column label="年龄" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.age }}</template>
        </el-table-column>
        <el-table-column label="入住日期" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.checkDate }}</template>
        </el-table-column>
        <el-table-column label="床位信息" align="center" min-width="120">
          <template slot-scope="scope">{{ elderForm.bedName }}</template>
        </el-table-column>
        <el-table-column label="就诊医生" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.doctor }}</template>
        </el-table-column>
        <el-table-column label="就诊日期" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.checkDate }}</template>
        </el-table-column>
        <el-table-column label="主诉内容" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.mainContent }}</template>
        </el-table-column>
        <el-table-column label="诊断结果" align="center" min-width="120">
        ...
        <el-table-column label="病程记录数" align="center" min-width="120">
  ...
  methods: {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/healthVisitRecords/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
