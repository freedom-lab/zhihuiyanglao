  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="药品名称" align="center" min-width="120" fixed="left">
          <template slot-scope="scope">{{ scope.row.drugName }}</template>
        </el-table-column>
        <el-table-column label="用药方式" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.useType | useTypeFormat }}</template>
        </el-table-column>
        <el-table-column label="用量" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.drugDosage }}</template>
        </el-table-column>
        <el-table-column label="药品来源" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="开始日期" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.startDate }}</template>
        </el-table-column>
        <el-table-column label="结束日期" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.endDate }}</template>
        </el-table-column>
        <el-table-column label="用药频次" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.drugFrequency }}</template>
        </el-table-column>
        <el-table-column label="早" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="早用量" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="中" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="中用量" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="晚" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="晚用量" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="睡前" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="睡前用量" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="药品审核状态" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.dd | sexFormat }}</template>
        </el-table-column>
        <el-table-column label="停药审核状态" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.dd | sexFormat }}</template>
        </el-table-column>
        <el-table-column label="药品状态" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.dd | sexFormat }}</template>
        </el-table-column>
        <el-table-column label="停用原因" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.dd }}</template>
        </el-table-column>
        <el-table-column label="记录人" align="center" min-width="120">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.post("/healthMedicineRecords/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/healthMedicineRecords/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/healthMedicineRecords/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
