  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="取餐号" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.mealNum }}</template>
        </el-table-column>
        <el-table-column label="餐品金额（元）" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.totalAmount }}</template>
        </el-table-column>
        <el-table-column label="实付金额（元）" align="center" min-width="120">
        ...
        <el-table-column label="支付方式" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.payMethod | payMethodFormat  }}</template>
        </el-table-column>
        <el-table-column label="订单来源" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.orderSource | orderSourceForamt }}</template>
        </el-table-column>
  ...
  methods: {
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insCanteenOrderInfo/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
