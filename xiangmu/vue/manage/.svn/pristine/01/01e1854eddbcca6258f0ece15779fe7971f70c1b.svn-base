      ...
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="模板名称" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.templetName }}</template>
          </el-table-column>
          <el-table-column label="模板编号" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.templetCode }}</template>
          </el-table-column>
          <el-table-column label="短信模板内容" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.smsContext }}</template>
          </el-table-column>
          <el-table-column label="模板类型（暂留）" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.templetType }}</template>
          </el-table-column>
        ...
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="模板名称：" prop="siteName">
                <el-input v-model="form.templetName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="模板编号：" prop="siteName">
                <el-input v-model="form.templetCode" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="短信模板内容：" prop="siteName">
                <el-input v-model="form.smsContext" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="模板类型（暂留）：" prop="siteName">
                <el-input v-model="form.templetType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/jcSmsTemplet/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/jcSmsTemplet/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/jcSmsTemplet/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
