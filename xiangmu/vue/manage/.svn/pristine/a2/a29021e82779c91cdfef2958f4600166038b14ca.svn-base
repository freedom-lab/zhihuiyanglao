      ...
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="模板类型" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.templateType }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="模板类型：" prop="siteName">
                <el-input v-model="form.templateType" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insStaffEvaluationTemplate/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insStaffEvaluationTemplate/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insStaffEvaluationTemplate/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
