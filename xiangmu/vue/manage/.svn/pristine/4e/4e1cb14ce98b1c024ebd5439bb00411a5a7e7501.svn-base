  ...
  <div style="padding:16px">
    ...
    <div class="table-box">
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%"
        ...
        <el-table-column label="消毒日期" min-width="110" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.disinfectDate ? scope.row.disinfectDate.slice(0, 10) : '-'
          }}</template>
        </el-table-column>
        <el-table-column label="消毒物品" min-width="130" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.deviceName || '-' }}</template>
        </el-table-column>
        <el-table-column label="消毒方法" min-width="130" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ scope.row.disinfectMethod || '-' }}</template>
        </el-table-column>
      ...
      <el-container class="dialogBox">
        ...
        <el-main class="dialogMain">
          <el-form ref="form" :model="form" :rules="rules" label-width="120px">
            <el-row :gutter="12">
              <el-col :span="12">
                <el-form-item label="消毒日期：" prop="disinfectDate">
                  <el-date-picker v-model="form.disinfectDate" type="date" placeholder="选择消毒日期"
                    style="width:100%"></el-date-picker>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="消毒物品：" prop="deviceName">
                  <el-input v-model="form.deviceName" placeholder="填写需要消毒的器具/物品"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="消毒方法：" prop="disinfectMethod">
                  <el-input v-model="form.disinfectMethod" placeholder="填写消毒操作方式"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="负责人：" prop="linkMan">
                  <el-input v-model="form.linkMan" placeholder="填写现场负责人姓名"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="联系电话：" prop="linkTel">
                  <el-input v-model="form.linkTel" placeholder="填写负责人联系手机号"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList() {
      var that = this;
      that.loading = true
      that.$http.post("/canteenKitchenDisinfect/list", that.queryInfo).then(function (response) {
    ...
    handleSave() {
      var that = this;
      that.$http.post("/canteenKitchenDisinfect/save", that.form).then(function (response) {
        if (response.data.code == 200) {
          that.$notify.success({
            title: "提示",
            message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/canteenKitchenDisinfect/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
