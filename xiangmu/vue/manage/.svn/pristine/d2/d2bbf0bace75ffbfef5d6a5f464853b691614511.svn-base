    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        <el-table-column label="序号" type="index" width="50" align="center"></el-table-column>
          ...
          <el-table-column label="执行日期" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.executeDate }}</template>
          </el-table-column>
          <el-table-column label="执行时间段" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.executeTime }}</template>
          </el-table-column>
          <el-table-column label="服药方法" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.drugMethods }}</template>
          </el-table-column>
          <el-table-column label="用药剂量" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.drugDosage }}</template>
          </el-table-column>
          <el-table-column label="用药单位" align="center" show-overflow-tooltip>
          ...
          <el-table-column label="执行说明/拒绝执行说明" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.executeMemo }}</template>
          </el-table-column>
          
          <el-table-column label="执行人姓名" align="center" show-overflow-tooltip>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="执行日期：" prop="siteName">
                <el-input v-model="form.executeDate" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="执行时间段：" prop="siteName">
                <el-input v-model="form.executeTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="服药方法：" prop="siteName">
                <el-input v-model="form.drugMethods" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="用药剂量：" prop="siteName">
                <el-input v-model="form.drugDosage" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="用药单位：" prop="siteName">
                <el-input v-model="form.drugUnit" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="执行状态：" prop="siteName">
                <el-input v-model="form.executeState" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="执行说明/拒绝执行说明：" prop="siteName">
                <el-input v-model="form.executeMemo" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
           
            <el-col :span="12">
              <el-form-item label="执行人姓名：" prop="siteName">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insMedicalDrugExecuteRecord/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insMedicalDrugExecuteRecord/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insMedicalDrugExecuteRecord/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
