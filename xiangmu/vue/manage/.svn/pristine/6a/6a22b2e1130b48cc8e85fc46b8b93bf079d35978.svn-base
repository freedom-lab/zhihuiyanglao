    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        ...
        <el-table-column label="老人姓名" align="center" show-overflow-tooltip>
          ...
          <el-table-column label="护理日期" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.nursingDate }}</template>
          </el-table-column>
          <el-table-column label="大类id" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.typeId }}</template>
          </el-table-column>
          <el-table-column label="大类名称" align="center" show-overflow-tooltip>
          ...
          <el-table-column label="护理状态(1未开始" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.nursingState }}</template>
          </el-table-column>
          <el-table-column label="护理完成时间" align="center" show-overflow-tooltip>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="护理日期：" prop="siteName">
                <el-input v-model="form.nursingDate" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="大类id：" prop="siteName">
                <el-input v-model="form.typeId" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="大类名称：" prop="siteName">
            ...
            <el-col :span="12">
              <el-form-item label="项目编号：" prop="siteName">
                <el-input v-model="form.itemCode" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="护理状态(1未开始：" prop="siteName">
                <el-input v-model="form.nursingState" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="护理完成时间：" prop="siteName">
            ...
            <el-col :span="12">
              <el-form-item label="护理图片：" prop="siteName">
                <el-input v-model="form.nursingPath" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="健康说明：" prop="siteName">
                <el-input v-model="form.healthInstructions" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insNursingTemporaryRecord/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insNursingTemporaryRecord/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insNursingTemporaryRecord/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
