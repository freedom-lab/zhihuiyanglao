  ...
  <div class="tab-box">
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column label="护理日期" align="center" min-width="100">
        ...
        <el-table-column label="状态" align="center" min-width="80">
          <template slot-scope="scope">{{ scope.row.nursingState | nursingStateFormat }}</template>
        </el-table-column>
        <el-table-column label="护理组" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.nursingGroupName }}</template>
        </el-table-column>
        <el-table-column label="护理人员" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.nursingStaffName }}</template>
        </el-table-column>
        <el-table-column label="护理等级" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.nursingLevelName }}</template>
        </el-table-column>
        <el-table-column label="护理类别" align="center" min-width="120">
          <template slot-scope="scope">{{ scope.row.typeName }}</template>
        </el-table-column>
        <el-table-column label="护理项目" align="center" min-width="100">
          <template slot-scope="scope">{{ scope.row.itemName }}</template>
        </el-table-column>
        <el-table-column label="图片" align="center" min-width="150" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-image v-for="item,index in scope.row.imgPathList" :key="index"
              :src="item" style="width: 60px;height: 60px;" :preview-src-list="[scope.row.imgPathList]"
            fit="contain">
...
export default {
  ...
  filters: {
    // 护理状态 1未开始 2服务中 3已完成 -1已撤销
    nursingStateFormat(val){
      switch (val) {
        case 1:
        case '1':
          return '未开始';
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.post("/insNursingExecutionRecord/list", that.queryInfo).then(function (response) {
        ...
        if (response.data.code == 200) {
          ...
          that.tableData = response.data.data.data.map(item=>{
            item.imgPathList = item.imgPath.split(',')
            return item
          });
        }
      });
    ...
    handleSave() {
      ...
      that.$refs.form.validate((valid) => {
        ...
        that.$http.post("/insNursingExecutionRecord/save", that.form).then( (response)=> {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "保存成功",
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
        var that = this;
        that.$http.post("/insNursingExecutionRecord/delete", { "id": id }).then(function (response) {
          if (response.data.code == 200) {
            that.$notify.success({
              title: "提示",
              message: "删除成功",
