    ...
    <div>
      <el-table border ref="table" v-loading="loading" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="height">
        ...
        <el-table-column label="老人姓名" align="center" show-overflow-tooltip>
          ...
          <el-table-column label="入住信息主键id" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.checkId }}</template>
          </el-table-column>
          ...
          <el-table-column label="康复训练计划名称" align="center" show-overflow-tooltip>
          ...
          <el-table-column label="执行状态(1未开始" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.executeState }}</template>
          </el-table-column>
          <el-table-column label="康复训练总结" align="center" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.summarizeContent }}</template>
          </el-table-column>
    ...
    <el-dialog :title="(labelType=='add'?'新增':'编辑')" :visible.sync="showDialog" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="康复训练计划名称：" prop="siteName">
                <el-input v-model="form.planName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="地点：" prop="siteName">
                <el-input v-model="form.locationName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="执行状态(1未开始：" prop="siteName">
                <el-input v-model="form.executeState" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="康复训练总结：" prop="siteName">
                <el-input v-model="form.summarizeContent" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="总结人id：" prop="siteName">
                <el-input v-model="form.nursingStaffId" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="总结人姓名：" prop="siteName">
                <el-input v-model="form.nursingStaffName" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="总结时间：" prop="siteName">
                <el-input v-model="form.summarizeTime" placeholder="请输入名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
	  var that = this;
    that.loading = true
	  that.$http.post("/insNursingRehabSetting/list", that.queryInfo).then(function (response) {
    ...
    handleSave () {
	    var that = this;
	  that.$http.post("/insNursingRehabSetting/save", that.form).then(function (response) {
		  if (response.data.code == 200) {
			  that.$notify.success({
				title: "提示",
				message: "保存成功",
...
//    },
    ...
    async handleDelte(id) {
      ...
      if (confirmResult === "confirm") {
           var that = this;
        that.$http.post("/insNursingRehabSetting/delete" ,{"id":id}).then(function (response) {
			if(response.data.code == 200){
			 that.$notify.success({
				title: "提示",
				message: "删除成功",
